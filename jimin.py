...
# ══════════════════════════════════════════════════════════════
최근_며칠     = 14     # 최근 며칠치를 볼지
국내보도_허용 = 5      # 국내 보도가 이 건수 이하면 통과
                       #   0 = 완전 미보도만  /  5 = 거의 안 다뤄진 것까지
                       #   네이버 검색에 엉뚱한 기사가 섞이는 일이 있어
                       #   3보다 조금 넉넉하게 잡았습니다
정밀검사_최대 = 80     # ★ 비용은 여기서만 결정됩니다. '사건' 개수입니다.
                       #   같은 사건을 받아쓴 기사는 몇 개든 1번만 검사합니다.
                       #   80개 ≈ 640원. 100으로 올리면 ≈ 800원.
매체당_최대   = 6      # 한 매체가 목록을 독차지하지 않게
                       #   3으로 조였더니 대상이 69건까지 줄었습니다.
...
검색어들 = [
    ...
    # ── 넓히면서 추가한 각도 ────────────────────────────
    # 성과·가십은 국내도 받아씁니다. 국내에 안 나오는 것은
    # 리뷰·분석·현지 문화 얘기 쪽에 몰려 있습니다.
...
지역들 = [
    ...
    ("프랑스",     "fr",    "FR", "FR:fr"),
    # ── 아래는 넓히면서 추가한 곳들 ──────────────────────
    # 국내에 거의 안 들어오는 권역입니다. 팬덤은 크고 한국어 보도는 없습니다.
    ("베트남",     "vi",    "VN", "VN:vi"),
    ("태국",       "th",    "TH", "TH:th"),
    ("말레이시아", "en-MY", "MY", "MY:en"),
    ("아르헨티나", "es-419", "AR", "AR:es-419"),
    ("콜롬비아",   "es-419", "CO", "CO:es-419"),
    ("칠레",       "es-419", "CL", "CL:es-419"),
    ("튀르키예",   "tr",    "TR", "TR:tr"),
    ("독일",       "de",    "DE", "DE:de"),
    ("이탈리아",   "it",    "IT", "IT:it"),
...
# ══════════════════════════════════════════════════════════════
#  판정 전에 같은 사건을 미리 묶습니다  ★ 비용이 여기서 갈립니다
# ══════════════════════════════════════════════════════════════
# 한 사건을 16개 매체가 받아쓰면, 예전에는 16번 모두 돈을 냈습니다.
# 이제 대표 1건만 검사하고 나머지는 그 결과를 물려받습니다.
# 같은 돈으로 훨씬 많은 '사건'을 볼 수 있습니다.
#
# 원문은 영어·일본어·스페인어가 섞여 있어 한국어처럼 다룰 수 없습니다.
# 그래서 언어를 안 가리는 두 가지 신호만 씁니다.
#   · 로마자 낱말 (Arlington, TikTok, cancer, kanker …)
#   · 한자·가나 두 글자 조각 (일본어 기사끼리 묶는 데 씁니다)

흔한말 = {"jimin", "bts", "kpop", "k-pop", "bangtan", "army", "the", "and", "for",
       "with", "from", "that", "this", "her", "his", "its", "los", "las", "del",
       "para", "por", "con", "que", "una", "dan", "yang", "untuk", "dari", "di",
       "news", "video", "photo", "watch", "fans", "star", "idol", "korean"}


# 일본어 지민 기사면 무조건 나오는 조각들. 이걸 안 빼면
# 전혀 다른 사건끼리 붙어 버립니다. (실제로 그랬습니다)
흔한조각 = {"ジミ", "ミン", "ジン", "韓流", "ンが", "ンの", "ンは", "ンに", "ンを",
        "アイ", "イド", "ドル", "グル", "ルー", "ープ", "メン", "ンバ", "バー",
        "ファ", "ァン", "公式", "話題", "反響", "報道", "発表", "人気", "世界",
        "今回", "自身", "投稿", "動画", "写真", "現在", "以降", "関連", "記事"}


def _표지(글):
    """언어를 안 가리는 사건 표지를 만든다."""
    낮 = (글 or "").lower()
    낱 = {w for w in re.findall(r"[a-z]{4,}", 낮) if w not in 흔한말}
    낱 |= set(re.findall(r"\d{2,}", 낮))          # 91,000 · 1억3000 같은 숫자
    for 덩이 in re.findall(r"[぀-ヿ一-鿿]{2,}", 글 or ""):
        낱 |= {덩이[i:i+2] for i in range(len(덩이) - 1)}
    return 낱 - 흔한조각


def _같은사건인가(a, b):
    """겹치는 비율로 봅니다. 개수만 보면 제목이 긴 쪽이 아무거나 끌어옵니다."""
    ㄱ, ㄴ = a["_표지"], b["_표지"]
    겹 = ㄱ & ㄴ
    if len(겹) < 2:
        return False
    닮음 = len(겹) / min(len(ㄱ), len(ㄴ))     # 짧은 쪽이 얼마나 덮이는가
    return 닮음 >= 0.45 and len(겹) >= 3


def 미리묶기(기사들, 날짜폭=10):
    """[[대표, 따라오는것...], ...] 로 만든다. 대표는 설명이 가장 충실한 것."""
    준비 = []
    for a in 기사들:
        b = dict(a)
        b["_표지"] = _표지(f'{a.get("제목","")} {a.get("설명","")}')
        준비.append(b)
    # 제목이 길수록 사건을 잘 설명합니다. 그걸 대표로 세웁니다.
    준비.sort(key=lambda a: -len(a.get("제목") or ""))
...
def 나라별_고르게(묶음들, 최대):
    """한 나라가 검사 예산을 독차지하지 않게 돌아가며 뽑는다."""
    통 = defaultdict(list)
    for m in 묶음들:
        통[m[0].get("나라", "?")].append(m)
    for 나라 in 통:                     # 나라 안에서는 최신순
        통[나라].sort(key=lambda m: m[0].get("날짜") or "", reverse=True)
    # 기사가 많은 나라부터 한 바퀴씩
    순서 = sorted(통, key=lambda n: -len(통[n]))
    고름 = []
    while len(고름) < 최대 and any(통[n] for n in 순서):
        for 나라 in 순서:
            if 통[나라]:
                고름.append(통[나라].pop(0))
                if len(고름) >= 최대:
                    break
    return 고름
...
def 판정하기(키, 기사들):
    # ① 같은 사건을 먼저 묶습니다. 돈은 '사건 수' 만큼만 듭니다.
    묶음 = 미리묶기(기사들)
    아낀것 = len(기사들) - len(묶음)

    # ② 한 나라가 예산을 독차지하지 않게 돌아가며 뽑습니다.
    대상 = 나라별_고르게(묶음, 정밀검사_최대)

    검사수 = len(대상)
    실린기사 = sum(len(m) for m in 대상)
    돈 = 검사수 * 8
    print(f"  [2/3] 국내에 나왔는지 확인하는 중...")
    print(f"        기사 {len(기사들)}건 → 사건 {len(묶음)}개"
          + (f" (받아쓴 {아낀것}건은 검사 안 합니다)" if 아낀것 else ""))
    print(f"        이번에 검사할 사건 {검사수}개 · 기사 {실린기사}건 · 약 {돈}원")
    if len(묶음) > 정밀검사_최대:
        print(f"        ※ 사건 {len(묶음)}개 중 {검사수}개만 봅니다. "
    ...
    for i, 무리 in enumerate(대상, 1):
        ...
        # 해외 기사 자체가 다른 지민이면 여기서 버린다 (따라오는 것들도 함께)
        ...
        공통 = {"성격": 정보.get("성격") or "?",
              ...
              "검색어": 정보.get("검색어") or [],
              "국내수": len(국내),
              "국내기사": list(국내.values())[:5],
              # 여기서 묶은 결과를 화면 쪽에 그대로 넘깁니다.
              # 물려받은 기사는 한글제목이 없어서, 이게 없으면
              # 화면에서 자기 무리를 못 찾습니다.
              "사건키": a.get("링크") or a.get("제목", "")[:60]}

        항목 = {**a, **공통,
              "한글제목": (정보.get("제목") or a["제목"])[:80],
              "요약": (정보.get("요약") or "")[:180]}

        # 같은 사건을 받아쓴 기사들 —— 검사 결과를 그대로 물려받습니다.
        # 한글제목은 비워 둡니다. 원문 제목이 그대로 보이는 편이 정직합니다.
        딸린것 = [{**b, **공통, "한글제목": "", "요약": ""} for b in 따라오는것]
        ...
        if len(국내) == 0:
            ...
            제외 += 묶은것; 표시 = "⚪"
        더 = f" +{len(따라오는것)}곳" if 따라오는것 else ""
        print(f"     {i:>3}. {표시} [{a['나라'][:4]}·국내{len(국내):>2}]{더} {a['제목'][:40]}")
