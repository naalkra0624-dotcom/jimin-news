...
# ★ '공연'은 지민 기사 거의 전부에 나오는 말입니다.
#   이것만 겹쳤다고 같은 사건으로 보면, 휴대폰 사고 · 테러 위협 · 립싱크 논란이
#   "콘서트"라는 말 하나로 한 덩어리가 됩니다. 실제로 사이트에서 그랬습니다.
#   그래서 '공연'은 혼자서는 근거가 못 되게 했습니다.
약한주제 = {"공연"}
...
def _같은사건(x, y):
    ...
    강한겹 = 겹 - 약한주제
    닮 = _닮음(x["_낱말"], y["_낱말"])
    # 제목이 아주 짧으면 낱말이 몇 개 안 됩니다.
    # 그때는 한 단어만 겹쳐도 닮음 값이 크게 나와 엉뚱한 것이 붙습니다.
    if min(len(x["_낱말"]), len(y["_낱말"])) < 4:
        return len(강한겹) >= 2         # 이럴 땐 뚜렷한 주제 두 개가 겹쳐야만
    if len(강한겹) >= 2:
        return True                     # 뚜렷한 주제 둘이 겹치면 같은 사건
    if len(강한겹) == 1 and 닮 >= 0.10:
        return True                     # 하나만 겹치면 말도 어느 정도 비슷해야
    if len(겹) >= 1 and 닮 >= 0.20:
        return True                     # 약한 주제뿐이면 말이 훨씬 비슷해야
    return 닮 >= 0.30                   # 주제가 안 잡혀도 말이 아주 비슷하면
...
def 사건묶기(기사들, 날짜폭=14):
    ...
    수집할 때 이미 묶인 것(사건키가 같은 것)은 그대로 존중하고,
    그 위에 한국어 요약으로 한 번 더 묶습니다. 수집 단계는 언어가 달라
    못 묶는 것이 있는데, 한국어 요약끼리는 묶이기 때문입니다.
    """
    # ── 1단계: 수집할 때 정해진 무리를 먼저 살린다 ──────────
    선묶음, 홀로 = {}, []
    for a in 기사들:
        키 = a.get("사건키")
        if 키:
            선묶음.setdefault(키, []).append(a)
        else:
            홀로.append(a)
    덩어리 = list(선묶음.values()) + [[a] for a in 홀로]

    # 각 덩어리의 대표(한글제목이 있는 것)를 세워 2단계에 넘긴다
    준비 = []
    for 덩 in 덩어리:
        ...
        while 바뀜:            # 무리에 붙은 기사를 기준으로 한 번 더 훑는다
            ...
            for a in 남:
                if any(abs((_날(a) - _날(b)).days) <= 날짜폭 and _같은사건(b, a)
                    ...
                    남은것.append(a)
            남, 남은것 = 남은것, []
        # 1단계에서 딸려 있던 식구들을 다시 펼친다
        펼침 = []
        for x in 무리:
            식구 = x.pop("_식구", [])
            x.pop("_낱말", None); x.pop("_주제", None)
            펼침.append(x)
            펼침 += 식구
        펼침[1:] = sorted(펼침[1:], key=lambda a: a.get("날짜") or "")
        묶음.append(펼침)
    return 묶음
